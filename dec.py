import os, sys, re, datetime, types, base64
try:
    import uncompyle6
...
have_code = base64.b64decode('IyBEZWNvbXBpbGUgYnkgOiBCaWxhbCBIYWlkZXIgSUQgCiMgVGltZSBTdWNjZXMgZGVjb21waWxlIDogJXMgCiVz')
...
def show_info(string):
    ...
    if type(ss) is types.CodeType:
        ...
        print '%s: No Compile Module given !!' % dah_lah
...
def dis(nama_file, output_file, ekse_file):
    ...
    if master_key.count('decompile eror?') != 0:
        if os.path.exists(output_file):
            ...
            exit('%s: Decompile failed!' % script_name)
    ...
    if master_key.count('exec') != 0:
        if len(re.findall('exec(.*)', master_key)) > 1:
            ...
            new_code = find_string_exec(master_key)
        show_info(new_code)
        ...
        exit('%s: decompile failed!. not found `exec`' % nama_file)
